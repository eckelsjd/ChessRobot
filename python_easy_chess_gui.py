...
def get_best_move(best_move: chess.Move, board: chess.Board):
    ...
    kingside = board.is_kingside_castling(best_move)
    queenside = board.is_queenside_castling(best_move)
    en_passant = board.is_en_passant(best_move)

    if (capture):
        captured_piece = board.piece_at(chess.parse_square(second_square.lower())).symbol()
        robot.set_fen(board.fen())
        robot.capture(first_square,second_square,captured_piece)
        ...
        # normal move
        robot.move(first_square,second_square)
